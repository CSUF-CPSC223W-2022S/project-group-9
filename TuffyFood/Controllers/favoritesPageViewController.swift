...
class favoritesPageViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var homeScreenVC: HomeScreenViewController?
    override func viewDidLoad() {
        super.viewDidLoad()
        favoritesTableView.delegate = self
        favoritesTableView.dataSource = self
        self.tabBarController!.viewControllers?[0] as? HomeScreenViewController
        logInVC = self.tabBarController!.viewControllers?[0] as? logInViewController
        addFavorite()
